...
- セル前後の空白を取り除く
- 日付表記を画面で選択した `yyyy/MM/dd`、`yyyy-MM-dd`、`yyyyMMdd` のいずれかにそろえる
- `¥12,000` のような金額表記を、画面で選択したカンマなしまたはカンマありの形式へ変換する
...
- 画面から選択したCSV文字列と加工設定（日付形式・金額形式）を `POST /api/clean` にJSONで送信
- `POST /api/clean` で `{ "csv": "...", "options": { "dateFormat": "yyyy/MM/dd", "amountFormat": "plain" } }` 形式のJSONを受け取り、`{ "csv": "...", "warnings": [...] }` 形式のJSONで返すAPI
- エラー時は `{ "error": "..." }` 形式のJSONで返却
- 入力CSVは最大1MBまでに制限し、超過時は 413 を返却
- CSV文字列のパース処理
- 空白トリム、選択された日付形式への統一、選択された金額形式への正規化の簡易加工
...
- 加工結果の差分表示
- サンプルCSVを使った自動テスト
- 主対象であるUTF-8 CSV以外の文字コード（Shift_JISなど）への対応
...
現時点ではUTF-8 CSVを主な対象としており、Shift_JISなどUTF-8以外の文字コード対応は今後の拡張予定です。

## ローカル実行方法

> 注意: MVP実装時点では、Codexによる `npm install` の実行や `package-lock.json` の作成は行っていません。

ローカルで実行する場合は、まず依存関係をインストールしてください。

```bash
npm install
```

その後、Cloudflare Workers の開発環境が整っている場合は、次のコマンドで起動する想定です。
...
公開用URLは準備中です。
現在は Cloudflare Workers 上で動作確認済みです。
